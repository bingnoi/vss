...
for ratio in ratios:
    ...
    if num > 0:
        # 生成不同范围的随机数
        low = min(random_numbers) + 1 if random_numbers else 1
        high = total_sum - sum(random_numbers) - (num_numbers - len(random_numbers))
        random_nums = np.random.randint(low, high + 1, size=num)
