...
class  TransformerZeroshotPredictor(nn.Module):
    def __init__(self,):
        ...
        if self.wordvec:
            ...
            self.bg_feature = nn.Parameter(torch.Tensor(1, 256))
        ...
        if self.wordvec:
            ...
            self.projection_layer = nn.Linear(hidden_dim, 256)
        ...
        if self.mask_classification:
            self.class_embed = nn.Linear(hidden_dim, 111 + 1)
    ...
    def forward(self, x, mask_features, images_tensor=None, ori_sizes=None):
        ...
        if self.mask_classification:
            x_cls = self.class_embed(hs)
            x_cls = x_cls / x_cls.norm(dim=-1, keepdim=True)
            logit_scale = self.logit_scale.exp()
            if self.training:
                print("ss",logit_scale.shape,x_cls.shape,self.text_features.clone().detach().t().shape)
