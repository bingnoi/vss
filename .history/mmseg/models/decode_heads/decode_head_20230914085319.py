...
class BaseDecodeHead_clips(nn.Module, metaclass=ABCMeta):
    ...
    def losses(self, seg_logit, images,seg_label):
        ...
        for i_label in split_group:
            ...
            ins = {}
            # print("ss",gt_classes.shape,gt_mask.shape)
            ins['gt_classes'] = gt_classes
            ins['gt_masks'] = gt_mask.squeeze()
            
            gt_classes.to('cuda' if torch.cuda.is_available() else 'cpu')
            ins['gt_masks'].to('cuda' if torch.cuda.is_available() else 'cpu')
            
            print('jj',gt_classes.device,ins['gt_masks'].device)
        ...
        # seg_logit['pred_logits'] = seg_logit['pred_logits'].cpu()
        # seg_logit['pred_masks']=seg_logit['pred_masks'].cpu()
