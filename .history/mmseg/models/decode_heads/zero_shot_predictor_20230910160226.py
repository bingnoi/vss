...
class  TransformerZeroshotPredictor(nn.Module):
    ...
    def forward(self, x, mask_features, images_tensor=None, ori_sizes=None):
        ...
        # print("mask",mask_features.shape)
        # exit()
        ...
        if self.mask_classification:
            ...
            print("ss1",cls_score.shape,logit_scale.shape, x_cls.shape , self.text_features.clone().detach().t().shape)
            print("ss2",bg_score.shape,logit_scale.shape, x_cls.shape , self.bg_feature.t().shape)
            
            outputs_class = torch.cat((cls_score, bg_score), -1)
            
            print("ss3",outputs_class.shape)
        ...
        # if self.aux_loss:
        #     # [l, bs, queries, embed]
        #     mask_embed = self.mask_embed(mask_features)
