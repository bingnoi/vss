...
class SegFormerHead_ZeroShot(BaseDecodeHead_clips):
    ...
    def forward(self,inputs, batch_size=None, num_clips=None):
        ...
        output=torch.cat([x,out1,out2,out3,out4],dim=0).squeeze(0)   ## b*(k+k)*124*h*w
        
        output_f = torch.cat(supp_feats,dim=1).squeeze(0)
        
        prediction = self.predictor(output,output_f).permute(0,3,1,2)
        
        prediction = F.interpolate(prediction,size=(h,w),mode='bilinear',align_corners=False).unsqueeze(0)
        
        # print("shape",output.shape,prediction.shape)
        # exit()

        if not self.training:
            mask_cls_results = prediction
            mask_pred_results = output_f
            # upsample masks
            print("size",mask_pred_results.shape,_c.shape[-2], _c.shape[-1])
            # exit()
            mask_pred_results = F.interpolate(
                mask_pred_results,
                size=(_c.shape[-2], _c.shape[-1]),
                mode="bilinear",
                align_corners=False,
            )
