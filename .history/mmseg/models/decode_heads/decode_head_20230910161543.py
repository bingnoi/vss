...
class BaseDecodeHead_clips(nn.Module, metaclass=ABCMeta):
    ...
    def losses(self, seg_logit, seg_label):
        ...
        print(self.hypercorre,self.cityscape)
        # torch.Size([1, 8, 124, 120, 120]) torch.Size([1, 4, 1, 480, 480])
        # torch.Size([60, 60, 112]) torch.Size([1, 4, 1, 480, 480])
        ...
        if self.hypercorre and self.cityscape:
            ...
            # elif seg_label.shape[1] == num_selected_by:
                ...
                seg_label_ori = seg_label_selected_all.reshape(batch_size*num_clips,1,h,w)
                seg_label_last3frame = seg_label_selected_last3.reshape(batch_size*(num_clips-1),-1,h,w)
                seg_label_lastframe = seg_label_selected_last.reshape(batch_size,-1,h,w)#每组最后那帧的预测
