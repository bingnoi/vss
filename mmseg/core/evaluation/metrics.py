...
class Evaluator(object):
    ...
    def _generate_matrix(self, gt_image, pre_image):
        # gt_image = gt_image - 112
        # print(np.unique(gt_image))
        mask = (gt_image >= 0) & (gt_image < self.num_class)
        label = self.num_class * gt_image[mask].astype('int') + pre_image[mask]
        # print(self.num_class,np.unique(gt_image[mask].astype('int')),np.unique(pre_image[mask]))
...
def intersect_and_union2(pred_label,
    ...
    # print(np.unique(label),np.unique(pred_label))
    # print("show",np.histogram(
    #      pred_label, bins=np.arange(num_classes + 1))[0])
    # print("shape",label.shape,pred_label.shape)
    ...
    # print('c1',np.unique(label),np.unique(pred_label),label.size,pred_label.size)
    # print('c2',np.unique(intersect),np.count_nonzero(intersect),intersect.size,intersect.shape)
    # exit()
...
def eval_metrics(results,
    ...
    # print(total_area_intersect[0],total_area_label.shape,acc.shape)
