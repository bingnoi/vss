...
class Pad(object):
    ...
    def __call__(self, results):
        ...
        # print(results['gt_semantic_seg'].min(), results['gt_semantic_seg'].max())
        # print(results['gt_semantic_seg'].shape, results['img'].shape)
        # exit()
        return results
...
class Pad_clips2(object):
    ...
    def _pad_img_clips2(self, results):
        """Pad images according to ``self.size``."""
        ...
        if self.size is not None:
            ...
            for im in results['img_beforeDistortion']:
                padded_img = mmcv.impad_to_multiple(
                    im, self.size_divisor, pad_val=self.pad_val)
                img_all.append(padded_img)
        results['img_beforeDistortion'] = img_all
        # results['pad_shape'] = padded_img.shape
        # results['pad_fixed_size'] = self.size
        # results['pad_size_divisor'] = self.size_divisor
...
class Normalize_clips2(object):
    ...
    def __call__(self, results):
        ...
        for im in results['img_beforeDistortion']:
            img = mmcv.imnormalize(im, self.mean, self.std,
                                          self.to_rgb)
            img_all_old.append(img)

        results['img']=img_all
        results['img_beforeDistortion']=img_all_old
        results['img_norm_cfg'] = dict(
            mean=self.mean, std=self.std, to_rgb=self.to_rgb)
        return results
...
@PIPELINES.register_module()
class PhotoMetricDistortion_clips2(object):
    """Apply photometric distortion to image sequentially, every transformation
    is applied with a probability of 0.5. The position of random contrast is in
    second or second to last.
