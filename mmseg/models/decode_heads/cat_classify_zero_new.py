...
class CatClassifier(nn.Module):
    def __init__(self,):
        ...
        text_template = build_text_prompt("vild")
        ...
        self.text_features = self.clip_adapter(self.class_texts)
        self.text_features_test = self.clip_adapter(self.test_class_texts)
    ...
    def forward(self, ori_images,fuse_f,c4,aux_query_out=None, images_tensor=None, ori_sizes=None):
        ...
        clip_features = self.clip_adapter.original_clip(ori_images, get_embedding=True)

        clip_features_bert = clip_features['final_cls_token']
        clip_features_bert = clip_features_bert / clip_features_bert.norm(dim=-1, keepdim=True)
        clip_features_bert = clip_features_bert.unsqueeze(1)
