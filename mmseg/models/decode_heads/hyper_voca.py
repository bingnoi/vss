...
class hypercorre_topk2(nn.Module):
    ...
    def forward(self, query_frame, supp_frame):
        ...
        atten_weight_2 = []
        for i in range(4):
            print(atten_all[i].shape,from_t_v[i].shape)
            atten_weight.append(torch.matmul(atten_all[i].permute(1,2,3,0),from_t_v[i]))
            atten_weight_1.append(torch.matmul(atten_all1[i].permute(1,2,3,0),last_features_cat6[i]))
            atten_weight_2.append(torch.matmul(atten_all2[i].permute(1,2,3,0),last_features_cat3[i]))
            print([i.shape for i in atten_weight],[i.shape for i in atten_weight_1],[i.shape for i in atten_weight_2])
