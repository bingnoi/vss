    ...
    def forward(self, x_all, mask_all=None, batch_size=None, num_clips=None):
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            ...
            for k in range(self.focal_level-1):
                ...
                #print(x_window_pooled.shape,self.qkv)          
        ...
        # # torch.Size([162, 8, 49, 32]) torch.Size([162, 8, 289, 32])
        # exit()
...
class CffmTransformerBlock3d3(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, expand_size=0, shift_size=0,
        ...
        self.downlinear = nn.Linear(208,dim,bias=qkv_bias)

    def forward(self, x,appearance):
        ...
        appearance = repeat(appearance,'b c h w->(b t) (h w) c',t=D0)
        
        # print(x.shape,appearance.shape)
        
        if appearance is not None:
            appearance = appearance.view(B0*D0,H0,W0,-1)
            x = torch.cat([x,appearance],dim=-1)

        # print(x.shape,self.downlinear)
        x = self.downlinear(x)
...
class BasicLayer3d3(nn.Module):
    ...
    def forward(self, x,img, batch_size=None, num_clips=None):
        B, C,T, H, W = x.shape
