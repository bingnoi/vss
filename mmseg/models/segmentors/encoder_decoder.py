...
class EncoderDecoder_clips(BaseSegmentor):
    ...
    def encode_decode(self, img, img_metas, batch_size, num_clips):
        ...
        # print("backbone",[i.shape for i in x])
        
        out = self._decode_head_forward_test(x,img, img_metas, batch_size, num_clips)
        # print("encode",out.shape)
        # if(out.shape[1]==512):
        #     exit()
        out = resize(
            input=out,
    ...
    def _decode_head_forward_test(self, x,img, img_metas, batch_size, num_clips):
        """Run forward function and calculate loss for decode head in
        inference."""
        
        seg_logits = self.decode_head.forward_test(x,img, img_metas, self.test_cfg, batch_size, num_clips)
        return seg_logits
    ...
    def simple_test(self, img, img_meta, rescale=True):
        ...
        # torch.Size([1, 124, 480, 853])
        # print(seg_logit[0,:,0,0])
        # exit()
        seg_pred = seg_logit.argmax(dim=1)
        ...
        seg_pred = seg_pred.cpu().numpy()
        import numpy as np
        # print("p",np.unique(seg_pred))
        # v,c=(np.unique(seg_pred,return_counts=True))
        # for k,q in zip(v,c):
        #     print(f"{k} occur {q}")
        # exit()
        # unravel batch dim
