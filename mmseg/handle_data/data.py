...
sorted_by_key = dict(sorted(my_dict.items(), key=lambda x: x[0].lower()))

# 按照数字大小排序
sorted_by_value = dict(sorted(my_dict.items(), key=lambda x: int(x[1])))
swapped_dict = {value: key for key, value in sorted_by_value.items()}
